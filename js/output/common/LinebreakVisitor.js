...
var LinebreakVisitor = (function (_super) {
    ...
    function LinebreakVisitor() {
        ...
        _this.FACTORS = {
            ...
            width: function (p) { return p + Math.floor((_this.state.width - _this.state.w) / _this.state.width * 2500); },
            tail: function (p) { return p + Math.floor(_this.state.width / Math.max(.0001, _this.state.mathLeft - _this.state.w) * 500); },
            open: function (p, mo) {
                var prevClass = mo.node.prevClass;
                if (prevClass === MmlNode_js_1.TEXCLASS.BIN || prevClass === MmlNode_js_1.TEXCLASS.REL || prevClass === MmlNode_js_1.TEXCLASS.OP) {
                    return p + 5000;
                }
                ...
                if ((parent === null || parent === void 0 ? void 0 : parent.isKind('mmultiscripts')) && mo.node === _this.getFirstToken(parent)) {
                    var prescripts = !!parent.childNodes.filter(function (node) { return node.isKind('mprescripts'); }).length;
                    if (prescripts)
    ...
    LinebreakVisitor.prototype.visitEmbellishedOperator = function (wrapper, _i) {
        ...
        if (style === 'before') {
            ...
            this.addWidth(bbox);
            var w = (style === 'after' ? 0 : mo.multChar ? mo.multChar.getBBox().w : bbox.w) + dw;
    ...
    LinebreakVisitor.prototype.visitMspaceNode = function (wrapper, i) {
        var bbox = wrapper.getLineBBox(i);
        var attributes = wrapper.node.attributes;
        if (attributes.getExplicit('height') === undefined &&
            attributes.getExplicit('depth') === undefined) {
            var penalty = this.mspacePenalty(wrapper);
    ...
    LinebreakVisitor.prototype.visitMsubsupNode = function (wrapper, i) {
        ...
        var w = Math.max(subbox.rscale * subbox.w, x + supbox.rscale * supbox.w) + msubsup.font.params.scriptspace;
        var _a = __read(this.getBorderLR(wrapper), 2), L = _a[0], R = _a[1];
